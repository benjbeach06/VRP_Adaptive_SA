...
| layer | file | job |
|---|---|---|
| core model | `SimAnn_VRP_Core_Model.py` | reports raw. The minimum data -- load and customer-count changes, start depot, vehicle -- needed to reconstruct the objective change and the cached values. |
...
| doc | what it covers |
|---|---|
| [raw_delta_record.md](raw_delta_record.md) | The record the core model produces: one map per field keyed by route; transition form; the one composition rule; why travel distance stays a single number. |
...
Per-route and per-vehicle distance, and end-depot usage as an objective term, are in
[planning/core-refactors/route-distance-tracking.md](../../planning/core-refactors/route-distance-tracking.md).
This pipeline makes both cheap to add.
...
- [planning/core-refactors/route-distance-tracking.md](../../planning/core-refactors/route-distance-tracking.md) -- per-route distance and end-depot usage, which this pipeline makes cheap to add
